   ...
   #     x = vmc(x,v)
    ...
    for m in range(M):
        ...
        if m > 0.1*M: #Only measure statistics after equilibration
            if m%m == 0: #Save values every other MC step to avoid having huge file
                sweeps+=1
                x = vmc(x,v)
                energy += (bh_kinetic(x,t,v) + bh_potential(x,U,mu))
                #print(energy/(sweeps))
                Egs.append(energy/sweeps)
                sweepsList.append(sweeps)
                
    print(len(sweepsList),len(Egs))

    #Format the data file
    columns = np.array((sweepsList,Egs))
    columns = columns.T
    with open("EGSbin_%d_%d_%.4f.dat"%(L,N,U),"w+") as data:
        np.savetxt(data,columns,delimiter="   ",fmt="%.16f",header="m      Egs     L=%d,N=%d,U=%.14f"%(L,N,U))
